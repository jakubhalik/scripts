...
int bin_search(int array[], int amount_of_numbers, int target) {
    int low = 0;

    int high = amount_of_numbers - 1;
...
int main() {
    ...
    output_string_and_int("The target: ", target);

    quicksort(array, 0, amount_of_numbers - 1);

    output_string_and_array("The sorted array: ", array, amount_of_numbers);

    int result = bin_search(array, amount_of_numbers, target);

    if (result != -1) {
        output_string_and_int("Target found at index: ", result);
    } else {
